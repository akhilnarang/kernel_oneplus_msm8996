...
static int32_t msm_sensor_driver_get_dt_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("%s support-front-camera : %s\n", __func__, s_ctrl->front_camera_name);

	if (sensordata->sensor_info->position == BACK_CAMERA_B) {
		if (of_property_read_u32(of_node, "qcom,pdaf-support", &is_pdaf_supported) < 0) {
